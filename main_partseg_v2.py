...
from data_v2 import ShapeNetPart
from model_octree_partseg_v9 import OctreeCaps_partseg
import numpy as np
...
def losshistory(log,epoch,name):
    ...
    csv_file_path = f"outputs/{args.exp_name}/models/{name}_log_{epoch}.csv"
...
def _init_():
    ...
    os.system('cp data_v2.py outputs' + '/' + args.exp_name + '/' + 'data_v2.py.backup')
...
def train(args, io):
    ...
    if args.model == 'octreecaps_part':
        seg_class_segcount = 50
        if args.class_choice is not None:
            seg_class_segcount =seg_num[list(np.where(args.class_choice)[0])[0]]
        model = OctreeCaps_partseg (args,output_channel=args.channels,seg_num=seg_class_segcount).to(device)
    else:
    ...
    best_test_iou = 0
    fold_results = {'train_loss': [], 'train_accuracy': [], 'Avg_Train_Accuracy': [],'train_iou': []}
    test_results = {'test_loss': [], 'test_accuracy': [], 'Avg_Test_Accuracy': []}

    directory_path = "outputs/{}/models/".format(args.exp_name)
    ...
    if largest_number is not None:
        ...
        checkpoint = torch.load(f"outputs/{args.exp_name}/models/model_{largest_number}.pt")
    ...
    for epoch in range(init_epoch,args.epochs):
        ...
        train_pred_seg = []
        train_label_seg = []
        for data,pointcloudlocation, label, seg in tqdm.tqdm(train_loader):
            ...
            data,pointcloudlocation, label_one_hot, seg = data.to(device),pointcloudlocation.to(device), label_one_hot.to(device), seg.to(device)
            #pointcloud = pointcloud.permute(0, 2, 1)

            opt.zero_grad()
            seg_pred = model(data,pointcloudlocation, label_one_hot)
            seg_pred = seg_pred.permute(0, 2, 1).contiguous()
            #print(f'seg_pred ={seg_pred.shape}')
            #print(f'seg label ={seg.shape}')
            loss = torch.nan_to_num(criterion(seg_pred.view(-1, seg_num_all), seg.view(-1, 1).squeeze(), mode='seg'), nan=3.7)
            ...
            print(f'pred ={pred[0]},seg= {seg[0]},loss = {loss.item()} ')
            print( f'train loss is = {loss.item()}, avg = {train_loss / count},acc= {torch.numel(torch.where(pred[0] == seg[0])[0]) / torch.numel(seg[0])}')
        ...
        if args.scheduler == 'cos':
            ...
            if opt.param_groups[0]['lr'] < 1e-5:
                for param_group in opt.param_groups:
                    param_group['lr'] = 1e-5
        train_loss_temp = train_loss / count
        ...
        for data, label, seg in tqdm.tqdm (test_loader):
            seg = seg - seg_start_index
            label_one_hot = np.zeros((label.shape[0], 16))  # 16 catergory
            for idx in range(label.shape[0]):
                label_one_hot[idx, label[idx]] = 1
            label_one_hot = torch.from_numpy(label_one_hot.astype(np.float32))
        ...
        if  epoch % 4 == 0:
            torch.save(model.state_dict(), f'outputs/{args.exp_name}/models/model_{epoch}.pt')
        ...
        fold_results['train_iou'].append(np.mean(train_ious))
        ...
        """
        vis.scatter(torch.tensor([epoch]), torch.tensor([avg_per_class_acc_test]), win='Test  Accuracy',
                    update='append',
...
def test(args, io):
    ...
    if args.model == 'dgcnn':
        model = OctreeCaps_partseg(args, output_channel=40, seg_part_all=seg_num_all).to(device)
    else:
        raise Exception("Not implemented")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, default='octreecaps_part', metavar='N',
                        choices=['dgcnn', 'dgccaps_part','octreecaps_part'],
