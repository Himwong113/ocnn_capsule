...
class OctreeKeyConv(nn.Module):
    def __init__(self,args,channel,lastlayer=6):
        ...
        self.dp1 = nn.Dropout(p=args.dropout)
        self.conv1 = nn.Sequential(nn.Conv3d(1, self.channel, kernel_size=(1,3,3),padding=(0,1,1),stride=(1,1,1), bias=False),
                                   self.bn1,
                                   nn.LeakyReLU(negative_slope=0.2),
                                   self.dp1,
...
class OctreeCaps(nn.Module):
    def __init__(self,args,channel=30):
        ...
        self.OctreeKeyConv_ver2 = OctreeKeyConv(channel=self.channel, args=args,lastlayer=self.cnnlastlayer)
    ...
    def forward(self,data):
        ...
        Latent = self.Latentcaps(dwrepeatvector.view(batch_size,1024,self.cnnlastlayer*(self.treedepth+1)))
