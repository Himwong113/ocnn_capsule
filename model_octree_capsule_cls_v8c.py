...
class OctreeKeyConv(nn.Module):
    def __init__(self, args, channel, lastlayer=6):
        ...
        self.conv2 = nn.Sequential(
            nn.Conv3d(self.channel, self.channel, kernel_size=(4, 3, 3), padding=(0, 1, 1), stride=(1, 1, 1),
                      bias=False),
            self.bn2,
            nn.LeakyReLU(negative_slope=0.2),
            self.dp2   ,
    ...
    def forward(self, x):
        ...
        x2 = self.conv1b(x1)
        #print(f'conv1 ={x2.shape}')
        x3 = self.conv1c(x2)
        #print(f'conv1 ={x3.shape}')
        x4 = self.conv1d(x3)
        #print(f'conv1 ={x4.shape}')
...
class OctreeCaps(nn.Module):
    def __init__(self, args, channel=30,classes =40):
        ...
        self.OctreeKeyConv = OctreeKeyConv(channel=self.classes, args=args)
    ...
    def forward(self, data):
        ...
        dwrepeatvector = dwrepeatvector.view(batch_size,  self.classes,256)
        ...
        #print(Latent.shape)
        result = self.linear(dwrepeatvector).view(batch_size,self.classes)
