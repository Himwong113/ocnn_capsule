...
class OctreeCaps_partseg(nn.Module):
    def __init__(self,args,output_channel =32,seg_num=50,latent_vec_size =64,N=3,h=3):
        ...
        self.decoder =build_decoder(d_model=self.h*latent_vec_size,
                                    N=N,
                                    h=h,
                                    dropout= args.dropout,
                                    num_point=args.num_points,
                                    cap_size=self.channel*2)
        self.conv = nn.Sequential(nn.Conv1d(in_channels=self.channel*2,out_channels=seg_num,kernel_size=1),
                                  nn.LeakyReLU(negative_slope=0.2),
                                  self.bn2,
                                  self.dp2)
    ...
    def forward(self,data,l):
        ...
        Latent = self.bn1a (self.Latentcaps(encoder))
        ...
        #print(f'l={l.shape}')
        caps_l = self.bn1b( self.Latentcaps_label(l))  # (batch_size, num_categoties, 1) -> (batch_size, channel, latent_vec_size) #batch 32,64
        #print(f'caps_l={caps_l.shape}')
        emb_vec =  torch.concat([Latent,caps_l],dim=1)
        """"
        try add bmm to decoder
        """
