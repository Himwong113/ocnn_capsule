...
class OctreeKeyConv(nn.Module):
    def __init__(self, args, channel):
        ...
        self.conv2 = nn.Sequential(
            nn.Conv1d(self.channel, self.channel, kernel_size=5, padding=0, stride=4, bias=False),
            self.bn2,
            nn.LeakyReLU(negative_slope=0.2),
            self.dp2)
        self.conv3 = nn.Sequential(
            nn.Conv1d(self.channel, self.channel, kernel_size=4, padding=0, stride=2, bias=False),
            self.bn3,
            nn.LeakyReLU(negative_slope=0.2),
            self.dp3)

        self.linear1 = nn.Sequential(
            nn.Conv1d(self.channel, self.channel, kernel_size=1, padding=0, stride=1, bias=False),
            self.bn4,
            nn.LeakyReLU(negative_slope=0.2),
...
class OctreeCaps_partseg(nn.Module):
    def __init__(self, args, output_channel=64, seg_num_all=50, latent_vec_size= 64, N=3, h=8):
        ...
        self.Latentcaps = LatentCapsLayer(prim_vec_size=self.channel,
                                          ...
                                          latent_vec_size= seg_num_all )


        """
        self.Latentcaps_label = LatentCapsLayer(prim_caps_size=16,
                                                prim_vec_size=1,
                                                latent_caps_size=self.channel,
                                                latent_vec_size=self.latent_vec_size)
        """

        """"
        self.decoder = build_decoder(d_model=self.latent_vec_size,
                                     N=N,
                                     h=h,
                                     dropout=args.dropout,
                                     num_point=args.num_points,
                                     cap_size=latent_vec_size)
        """

        self.bn1 = nn.BatchNorm1d(128)
        self.bn2 = nn.BatchNorm1d(256)
        self.bn3 =  nn.BatchNorm1d(512)
        self.bn4 = nn.BatchNorm1d(self.num_points)

        self.dp1 = nn.Dropout(args.dropout)
        self.dp2 = nn.Dropout(args.dropout)
        self.dp3 = nn.Dropout(args.dropout)
        self.dp4 = nn.Dropout(args.dropout)

        # in : batch x 64x50
        self.conv1 = nn.Sequential(nn.Conv1d(in_channels=latent_vec_size, out_channels=128, kernel_size=1),
                                  nn.LeakyReLU(negative_slope=0.2),
                                  self.bn1,
                                  self.dp1)
        self.conv2 = nn.Sequential(nn.Conv1d(in_channels=128, out_channels=256, kernel_size=1),
                                   nn.LeakyReLU(negative_slope=0.2),
                                   self.bn2,
                                   self.dp2)
        self.conv3 = nn.Sequential(nn.Conv1d(in_channels=256, out_channels=512, kernel_size=1),
                                   nn.LeakyReLU(negative_slope=0.2),
                                   self.bn3,
                                   self.dp3)
        self.conv4 = nn.Sequential(nn.Conv1d(in_channels=896, out_channels=self.num_points, kernel_size=1),
                                   nn.LeakyReLU(negative_slope=0.2),
                                   self.bn4,
                                   self.dp4)
    ...
    def forward(self, data, l):
        ...
        #print(data.shape)
        encoder = self.OctreeKeyConv(data)#.permute((0,2,1))  # batch 1287,32
        #print(f'encoder ={encoder.shape}')
        out = self.Latentcaps(encoder)
        #print(f'latent = {out.shape}')  # batch ,40,40

        out1 =   self.conv1(out)
        out2 =   self.conv2(out1)
        out3 =   self.conv3(out2)
        out = torch.concat([out1,out2,out3],dim=1)
        #print(f'cncat out ={out.shape}')
        out =  F.softmax( self.conv4(out),dim=-1)
        out = torch.permute(out,(0,2,1))

        # result = torch.bmm(encoder, Latent).permute((0,2,1))
        # print(result.shape)  #12, 40,1287
        ...
        #print(f'l={l.shape}')
        #caps_l = self.Latentcaps_label(
        #    l)  # (batch_size, num_categoties, 1) -> (batch_size, channel, latent_vec_size) #batch 32,64
        #print(f'caps_l={caps_l.shape}')
        #emb_vec = torch.concat([Latent, caps_l], dim=1)
        #print(f'embsize = {emb_vec.shape}')
#
        #emb_vec = emb_vec.repeat(1, 1, self.h)  # (batch_size, latent_vec, seg_allxhead) #batch 32,50x3 -> batch 32,150
        ## print(f'embsize = {emb_vec.shape}')

        #decoder = self.decoder(encoder)
        #print(f'decode ={decoder.shape}')
...
if __name__ == "__main__":
    ...
    model = OctreeCaps_partseg(args).to('cuda')
