...
class OctreeCaps_partseg(nn.Module):
    def __init__(self,args,
        ...
        self.bn1b = nn.BatchNorm1d(self.pointcloud_channel)

        self.pointcloud_conv = nn.Sequential(nn.Conv1d(3, self.pointcloud_channel, kernel_size=1, bias=False),
                                             nn.LeakyReLU(negative_slope=0.2),
                                             self.bn1b,
                                             self.dp1b)
        ...
        self.conv2 = nn.Sequential(nn.Conv1d(1287+self.label_channel+self.num_points, 1024, kernel_size=1, bias=False),
                                   nn.LeakyReLU(negative_slope=0.2),
                                   self.bn2,
                                   self.dp2)
    ...
    def forward(self,data,pointcloud,l):
        ...
        #result = self.bn0 (torch.bmm(encoder, Latent))
        result = encoder.max(dim=-1,keepdim=True)[0]
        #print(f'result={result.shape}')  #12, 40,1287

        """
        pointcloud_embedd = self.pointcloud_embedd(pointcloud_embedd)
        pointcloud_embedd = self.bn1b(pointcloud_embedd)
        """

        pointcloud_embedd = self.pointcloud_conv(pointcloud)
        pointcloud_embedd = torch.permute(pointcloud_embedd,(0,2,1))
        pointcloud_embedd = pointcloud_embedd.max(dim=-1,keepdim=True)[0]
        ...
        print(f'l={l.shape}')
        caps_l = self.label_embedd(l)  # (batch_size, num_categoties, 1) -> (batch_size, channel, latent_vec_size) #batch 32,64
        print(f'caps_l={caps_l.shape}')
        emb_vec =  torch.concat([result,pointcloud_embedd,caps_l],dim=1)
