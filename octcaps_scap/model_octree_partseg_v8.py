...
class LatentCapsLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        u_hat = torch.squeeze(torch.matmul(W, x[:, None, :, :, None]), dim=-1)
...
class OctreeCaps_partseg(nn.Module):
    def __init__(self,args,output_channel=60,
        ...
        self.Latentcaps = LatentCapsLayer(
            prim_caps_size=self.node_count,
            prim_vec_size=self.octree_channel,
            latent_caps_size=self.octree_channel,
...
if __name__=="__main__":
    ...
    pcd = torch.randint(0,2,(args.batch_size, 1024,3)).to('cuda').float()
    print(pcd)
    out = model(x,pcd,l)
    print(out.shape)
